...
def list_all_accounts(db: DBSession):
    ...
    _accounts: Sequence[FreezeLogAccount] = db.scalars(select(FreezeLogAccount)).all()
...
def delete_account(db: DBSession, account_address: str):
    ...
    _account: FreezeLogAccount | None = db.scalars(
...
def record_new_log(
    ...
    # Search for an account
    log_account: FreezeLogAccount | None = db.scalars(
        select(FreezeLogAccount)
    ...
    try:
        _, log_index = log_contract.record_log(
...
def retrieve_log(
    ...
    block_number, freezing_grace_block_count, log_message = log_contract.get_log(
        log_index=log_index
    )
