...
class CreateUpdateBondLedgerTemplateRequestJPN(BaseModel):
    ...
    ledger_admin_headquarters: str
    ledger_admin_office_address: str
    ...
    @validator("ledger_admin_headquarters")
    def ledger_admin_headquarters_length_is_less_than_200(cls, v):
        if len(v) > 200:
            raise ValueError("The length must be less than or equal to 200")
        return v

    @validator("ledger_admin_office_address")
    def ledger_admin_office_address_length_is_less_than_200(cls, v):
        if len(v) > 200:
            raise ValueError("The length must be less than or equal to 200")
        return v
