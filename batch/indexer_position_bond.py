...
class Processor:
    ...
    async def sync_new_logs(self):
        ...
        try:
            ...
            # Update the latest block number in IDXPositionBondBlockNumber
    ...
    async def __get_idx_position_block_number(db_session: AsyncSession) -> int:
        _idx_position_block_number: IDXPositionBondBlockNumber | None = (
            await db_session.scalars(select(IDXPositionBondBlockNumber).limit(1))
        ).first()
    ...
    async def __get_account_balance_all(
        ...
        bond_token = IbetStraightBondContract(token_address)
        await bond_token.get()
