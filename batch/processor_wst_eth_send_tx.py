...
async def get_tx_sender_account(
    ...
    if tx_sender == ETH_MASTER_ACCOUNT_ADDRESS:
        ...
        assert ETH_MASTER_PRIVATE_KEY is not None
...
async def send_deploy_transaction(
    ...
    tx_hash, nonce = await EthAsyncContractUtils.deploy_contract(
...
async def send_add_whitelist_transaction(
    ...
    wst_contract = EthereumIbetWST(wst_tx.ibet_wst_address)
