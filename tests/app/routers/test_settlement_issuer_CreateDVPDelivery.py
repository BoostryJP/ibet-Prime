...
def settlement_blockchain_mocks() -> Generator[None, None, None]:
    async def forced_transfer(*, tx_params: object, **_: object) -> str:
        if getattr(tx_params, "amount", 0) >= 1000:
            raise SendTransactionError("setup transaction error")
        return "forced_transfer_tx_hash"

    with patch(
        "app.routers.issuer.settlement_issuer.IbetSecurityTokenInterface.forced_transfer",
        new=AsyncMock(side_effect=forced_transfer),
    ):
        yield
...
class TestCreateDVPDelivery:
    ...
    async def test_normal_1_1(
        ...
        resp = await async_client.post(
            self.base_url.format(exchange_address=DVP_CONTRACT_ADDRESS),
            json=req_param,
            headers={
        ...
        assert _async_process.dvp_contract_address == DVP_CONTRACT_ADDRESS
        assert _async_process.token_address == TOKEN_CONTRACT_ADDRESS
    ...
    async def test_normal_2(
        ...
        assert _async_process.data is not None
