...
def build_mock_share_contract(
    ...
    contract.issue_price = token_create_param["issue_price"]
    contract.cancellation_date = token_create_param["cancellation_date"]
    contract.memo = ""
    contract.principal_value = token_create_param["principal_value"]
    contract.is_canceled = token_create_param["is_canceled"]
    contract.dividends = token_create_param["dividends"]
    contract.dividend_record_date = token_create_param["dividend_record_date"]
    contract.dividend_payment_date = token_create_param["dividend_payment_date"]
    return contract


async def prepare_share_token_history_fixture(
...
) -> tuple[IbetShareContract, dict[str, Any]]:
    token_address = TEST_SHARE_TOKEN_ADDRESS
    ...
    return (
        build_mock_share_contract(address, token_address, token_create_param),
        token_create_param,
    )
...
class TestAppRoutersShareTokensTokenAddressHistoryGET:
    ...
    async def create_history_by_api(
        ...
        base_contract = build_mock_share_contract(
            issuer_address, token_address, create_param
        )
        dividends_contract = build_mock_share_contract(
            issuer_address, token_address, create_param
        )
        dividends_contract.dividends = 1.0
        dividends_contract.dividend_record_date = "20230502"
        dividends_contract.dividend_payment_date = "20230502"
        memo_contract = build_mock_share_contract(
            issuer_address, token_address, create_param
        )
        memo_contract.dividends = 1.0
        memo_contract.dividend_record_date = "20230502"
        memo_contract.dividend_payment_date = "20230502"
        memo_contract.memo = "." * 10000

        with (
            patch(
                "app.model.ibet.token.IbetShareContract.get",
                side_effect=[base_contract, dividends_contract, memo_contract],
            ),
            patch(
    ...
    async def test_normal_2(
        ...
        # Prepare data : Token
        token_contract, create_param = await prepare_share_token_history_fixture(
            async_db,
            _issuer_address,
        ...
        _token = Token()
        _token.token_address = token_contract.token_address
        _token.issuer_address = _issuer_address
        _token.type = TokenType.IBET_SHARE
        _token.tx_hash = ""
        _token.abi = {}
    ...
    async def test_normal_3_3(
        ...
        # Prepare data : Token
        token_contract, _ = await prepare_share_token_history_fixture(
            async_db,
            _issuer_address,
            TEST_PERSONAL_INFO_CONTRACT_ADDRESS,
