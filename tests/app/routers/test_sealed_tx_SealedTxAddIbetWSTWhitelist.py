...
@pytest.mark.asyncio
class TestSealedTxAddIbetWSTWhitelist:
    api_url = "/sealed_tx/ibet_wst/whitelists/add"

    issuer = default_eth_account("user1")
    delegate = default_eth_account("user2")
    user1_st = default_eth_account("user3")
    user1_sc = default_eth_account("user4")
    relayer = default_eth_account("user5")
    ...
    # <Normal_1>
    # Add account to IbetWST whitelist by sealed tx
    # - blockchain_platform = "ethereum" (default)
    @mock.patch(
        "app.routers.misc.sealed_tx.ETH_MASTER_ACCOUNT_ADDRESS",
        relayer["address"],
    )
