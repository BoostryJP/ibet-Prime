...
class TestGetContractCode:
    ...
    async def test_normal_1(self):
        ...
        (
            ...
            rtn_bytecode,
            rtn_deploy_bytecode,
        ) = AvaAsyncContractUtils.get_contract_code(contract_name="AuthIbetWST")
    ...
    async def test_error_1(self):
        with pytest.raises(FileNotFoundError):
            AvaAsyncContractUtils.get_contract_code(contract_name="not_exist_contract")
...
class TestDeployContract:
    ...
    async def test_normal_1(self):
        # Deploy contract
        tx_hash, _ = await AvaAsyncContractUtils.deploy_contract(
    ...
    async def test_error_1(self):
        with pytest.raises(SendTransactionError):
            await AvaAsyncContractUtils.deploy_contract(
    ...
    async def test_error_2(self):
        send_tx_mock = patch(
            target="app.utils.ava_contract_utils.AvaAsyncContractUtils.send_transaction",
...
class TestGetContract:
    ...
    async def test_error_1(self):
        ...
        # Get contract
        with pytest.raises(FileNotFoundError):
            AvaAsyncContractUtils.get_contract(
                contract_name="NOT_EXIST_CONTRACT",
                contract_address=contract_address,
            )
...
class TestCallFunction:
    ...
    async def test_normal_1(self):
        ...
        # Call a function
        owner_address = await AvaAsyncContractUtils.call_function(
            contract=contract,
            function_name="owner",
            args=(),
        )
    ...
    async def test_error_1_1(self):
        ...
        with pytest.raises(Web3Exception):
            await AvaAsyncContractUtils.call_function(
                contract=contract,
...
class TestGetBlockByTransactionHash:
    ...
    async def test_normal_1(self):
        ...
        # Get block by transaction hash
        block = await AvaAsyncContractUtils.get_block_by_transaction_hash(tx_hash)
...
class TestGetFinalizedBlockNumber:
    ...
    async def test_normal_1(self):
        ...
        latest_block = await AvaAsyncContractUtils.get_block_by_transaction_hash(
            tx_hash
        )
...
class TestGetEventLogs:
    ...
    async def test_normal_1(self):
        ...
        tx_hash, _ = await AvaAsyncContractUtils.send_transaction(
            transaction=tx,
