...
def processor(async_db, caplog: pytest.LogCaptureFixture):
    ...
    log.propagate = True
    yield AvaWSTBridgeMonitoringProcessor()
    log.propagate = False
...
class TestProcessor:
    ...
    @mock.patch(
        "batch.processor_wst_ava_monitor_bridge_events.AVA_MASTER_ACCOUNT_ADDRESS",
        relayer["address"],
    )
    @mock.patch(
        "batch.processor_wst_ava_monitor_bridge_events.AvaBridgeEventViewer.get_ibet_event_logs",
        AsyncMock(
            side_effect=[
                [
                    {
    ...
    @mock.patch(
        "batch.processor_wst_ava_monitor_bridge_events.AvaBridgeEventViewer.get_wst_event_logs",
        AsyncMock(
            side_effect=[
                [],  # __process_burn
                [],  # __process_transfer
    ...
    async def test_normal_1_2(self, processor, async_db, caplog):
        ...
        # Check AvaIbetWSTTx
        eth_tx_list: Sequence[AvaIbetWSTTx] = (
            await async_db.scalars(
        ...
        # Check AvaToIbetBridgeTx
        ibet_tx_list: Sequence[AvaToIbetBridgeTx] = (
            await async_db.scalars(select(AvaToIbetBridgeTx))
        ).all()
    ...
    async def test_normal_2(self, processor, async_db, caplog):
        ...
        synced_block_eth = IbetWSTBridgeSyncedBlockNumber(
            network="avalanche", latest_block_number=888888888
        )
