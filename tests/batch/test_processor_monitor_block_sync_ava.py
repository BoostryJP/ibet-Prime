...
from batch.processor_monitor_block_sync_ava import Processor
...
def processor(async_db: AsyncSession, monkeypatch: pytest.MonkeyPatch):
    import batch.processor_monitor_block_sync_ava as processor_module
...
class TestProcessor:
    ...
    @mock.patch(
        "batch.processor_monitor_block_sync_ava.AVA_WEB3_HTTP_PROVIDER",
    ...
    async def test_normal_1(self, processor: Processor, async_db: AsyncSession):
        ...
        with mock.patch(
            "batch.processor_monitor_block_sync_ava.BLOCK_GENERATION_SPEED_THRESHOLD",
            100,
        ):
            await processor.process()
            await async_db.rollback()
        ...
        node_web3 = processor.node_info[AVA_WEB3_HTTP_PROVIDER]["web3"]
    ...
    @mock.patch(
        "batch.processor_monitor_block_sync_ava.AVA_WEB3_HTTP_PROVIDER_STANDBY",
        ["http://localhost:1000"],
    )
    @pytest.mark.asyncio
    ...
    async def test_normal_3(self, processor: Processor, async_db: AsyncSession):
        ...
        # assertion-1
        old_node = (await async_db.scalars(select(AvalancheNode))).all()
    ...
    async def test_error_1(self, processor: Processor, async_db: AsyncSession):
        ...
        # assertion
        _node_list = (
            await async_db.scalars(select(AvalancheNode).order_by(AvalancheNode.id))
        ).all()
