...
from batch.processor_monitor_block_sync_ibet import Processor
...
class FakeWeb3:
    def __init__(self, provider: Any):
        ...
        self.middleware_onion = SimpleNamespace(inject=inject)


@pytest.fixture(scope="function")
def processor(async_db: AsyncSession, monkeypatch: pytest.MonkeyPatch):
    import batch.processor_monitor_block_sync_ibet as processor_module
...
class TestProcessor:
    ...
    @mock.patch(
        "batch.processor_monitor_block_sync_ibet.WEB3_HTTP_PROVIDER",
        WEB3_HTTP_PROVIDER,
    )
    @pytest.mark.asyncio
    async def test_normal_1(self, processor: Processor, async_db: AsyncSession):
        ...
        _node: Any = (await async_db.scalars(select(Node).limit(1))).first()
        assert _node.id == 1
        ...
        with mock.patch(
            "batch.processor_monitor_block_sync_ibet.BLOCK_GENERATION_SPEED_THRESHOLD",
            100,
        ):
            await processor.process()
            await async_db.rollback()
        ...
        node_web3 = processor.node_info[WEB3_HTTP_PROVIDER]["web3"]
    ...
    @mock.patch(
        "batch.processor_monitor_block_sync_ibet.WEB3_HTTP_PROVIDER_STANDBY",
        ["http://localhost:1000"],
    )
    @pytest.mark.asyncio
    async def test_normal_2(self, processor: Processor, async_db: AsyncSession):
        ...
        provider.endpoint_uri = WEB3_HTTP_PROVIDER  # Temporarily replace setting values
        await processor.process()
        await async_db.rollback()
        async_db.expire_all()
    ...
    async def test_normal_3(self, processor: Processor, async_db: AsyncSession):
        ...
        # assertion-2
        new_node: Any = (await async_db.scalars(select(Node).limit(1))).first()
