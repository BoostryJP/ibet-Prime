...
class TestAppRoutersShareTokensTokenAddressForceUnlockBatchPOST:
    # target API endpoint
    ...
    def test_normal_1(self, client, db):
        ...
        test_account_3 = config_eth_account("user4")["address"]
        ...
        req_param = [
            {
                ...
                "lock_address": test_account_2,
                "recipient_address": test_account_3,
                "value": 10,
            }
        ]
        resp = client.post(
        ...
        # assertion
        upload: Optional[BatchForceUnlockUpload] = db.query(
            BatchForceUnlockUpload
        ).first()
        ...
        batch_data_list: list[BatchForceUnlock] = db.query(BatchForceUnlock).all()
        assert len(batch_data_list) == 1

        batch_data_1: BatchForceUnlock = batch_data_list[0]
        assert batch_data_1.batch_id == upload.batch_id
        assert batch_data_1.account_address == req_param[0]["account_address"]
        assert batch_data_1.lock_address == req_param[0]["lock_address"]
        assert batch_data_1.recipient_address == req_param[0]["recipient_address"]
        assert batch_data_1.value == req_param[0]["value"]
        assert batch_data_1.status == 0

        assert resp.status_code == 200
        assert resp.json() == {"batch_id": upload.batch_id}
    ...
    def test_normal_2(self, client, db):
        ...
        test_account_2 = config_eth_account("user3")["address"]
        test_account_3 = config_eth_account("user4")["address"]
        test_account_4 = config_eth_account("user5")["address"]
        ...
        req_param = [
            ...
            {
                "account_address": test_account_2,
                "lock_address": test_account_3,
                "recipient_address": test_account_4,
                "value": 10,
            },
        ]
        resp = client.post(
        ...
        batch_data_2: BatchForceUnlock = batch_data_list[1]
        assert batch_data_2.batch_id == upload.batch_id
        assert batch_data_2.account_address == req_param[1]["account_address"]
        assert batch_data_2.lock_address == req_param[1]["lock_address"]
        assert batch_data_2.recipient_address == req_param[1]["recipient_address"]
        assert batch_data_2.value == req_param[1]["value"]
